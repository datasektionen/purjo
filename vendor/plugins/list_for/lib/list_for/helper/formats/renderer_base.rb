    ...
    module Formats
      class RendererBase
        ...
        def add_to_uri(uri, params)
          ...
          query_hash = if defined?(CGIMethods) || defined?(ActionController::AbstractRequest)
            (defined?(CGIMethods) ? CGIMethods : ActionController::AbstractRequest).parse_query_parameters(uri_copy.query)
          elsif defined?(Rack::Utils)
            Rack::Utils.parse_query(uri_copy.query)
          end
